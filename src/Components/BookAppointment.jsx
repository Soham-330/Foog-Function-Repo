...
function BookAppointment(props) {
    ...
    const [lifeCoachs, setLifeCoachs] = useState([]);
    ...
    useEffect(() => {
        ...
        const fetchLifeCoach = async () => {
            try {
                const q = query(
                    collection(db, 'dietician'),
                    where('category','==','lifecoach'),
                  );
                const querySnapshot = await getDocs(q);
                const fetchedLifeCoachs = querySnapshot.docs.map(doc =>({ id: doc.id, ...doc.data() }));
                setLifeCoachs(fetchedLifeCoachs);
            } catch (error) {
                console.error("Error fetching Life Coaches: ", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <>
            ...
            <div className="appointment-body">
                <div className="container">
                    <div className="description">
                        <h2 className='heading'>LifeCoach Services</h2>
                        <p>Book a one-on-one consultation with a certified dietician to discuss your dietary needs and health goals. Our dieticians provide personalized advice, meal planning, and practical strategies to help you achieve better health.
                        </p>
                        <div>
                            <b>Benefits:</b>
                    ...
                    <div className="carousel">
                        <Carousel showArrows={true} showThumbs={false} showStatus={false}>
                            {lifeCoachs.map((dietician, index) => (
                                <div className='doc-carousel' key={index}>
                                    <img src={dietician.image} alt={`Dietician ${dietician.name}`} />
                                    <h3>{dietician.name}</h3>
                                    <ul>
